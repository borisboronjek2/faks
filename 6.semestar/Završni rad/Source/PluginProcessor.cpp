...
//==============================================================================
MultibandAudioCompressorAudioProcessor::MultibandAudioCompressorAudioProcessor()
...
//==============================================================================
const juce::String MultibandAudioCompressorAudioProcessor::getName() const
{
    return JucePlugin_Name;
}

bool MultibandAudioCompressorAudioProcessor::acceptsMidi() const
{
...
bool MultibandAudioCompressorAudioProcessor::producesMidi() const
{
...
bool MultibandAudioCompressorAudioProcessor::isMidiEffect() const
{
   #if JucePlugin_IsMidiEffect
    return true;
   #else
...
double MultibandAudioCompressorAudioProcessor::getTailLengthSeconds() const
{
    return 0.0;
}

int MultibandAudioCompressorAudioProcessor::getNumPrograms()
{
    return 1;   // NB: some hosts don't cope very well if you tell them there are 0 programs,
                // so this should be at least 1, even if you're not really implementing programs.
}

int MultibandAudioCompressorAudioProcessor::getCurrentProgram()
{
    return 0;
}

void MultibandAudioCompressorAudioProcessor::setCurrentProgram (int index)
{
}

const juce::String MultibandAudioCompressorAudioProcessor::getProgramName (int index)
{
    return {};
}

void MultibandAudioCompressorAudioProcessor::changeProgramName (int index, const juce::String& newName)
{
}

//==============================================================================
void MultibandAudioCompressorAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
...
void MultibandAudioCompressorAudioProcessor::releaseResources()
{
...
#ifndef JucePlugin_PreferredChannelConfigurations
bool MultibandAudioCompressorAudioProcessor::isBusesLayoutSupported (const BusesLayout& layouts) const
{
  #if JucePlugin_IsMidiEffect
...
void MultibandAudioCompressorAudioProcessor::updateState()
{
    ...
    auto lowMidCuttofFreq = lowMidCrossover->get();
    LP1.setCutoffFrequency(lowMidCuttofFreq);
    HP1.setCutoffFrequency(lowMidCuttofFreq);

    auto midHighCuttofFreq = midHighCrossover->get();
    AP2.setCutoffFrequency(midHighCuttofFreq);
    LP2.setCutoffFrequency(midHighCuttofFreq);
    HP2.setCutoffFrequency(midHighCuttofFreq);
...
void MultibandAudioCompressorAudioProcessor::splitBands(const juce::AudioBuffer<float>& inputBuffer)
{
...
void MultibandAudioCompressorAudioProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
...
//==============================================================================
bool MultibandAudioCompressorAudioProcessor::hasEditor() const
{
    return true; // (change this to false if you choose to not supply an editor)
}

juce::AudioProcessorEditor* MultibandAudioCompressorAudioProcessor::createEditor()
{
    return new MultibandAudioCompressorAudioProcessorEditor (*this);
}

//==============================================================================
void MultibandAudioCompressorAudioProcessor::getStateInformation (juce::MemoryBlock& destData)
{
...
void MultibandAudioCompressorAudioProcessor::setStateInformation (const void* data, int sizeInBytes)
{
...
juce::AudioProcessorValueTreeState::ParameterLayout MultibandAudioCompressorAudioProcessor::createParamaterLayout()
{
    ...
    layout.add(std::make_unique<AudioParameterFloat>(params.at(Names::Gain_In),
                                                     ...
                                                     NormalisableRange<float>(-24.f, 24.f, 0.5f, 1.f),
                                                     0));
    layout.add(std::make_unique<AudioParameterFloat>(params.at(Names::Gain_Out),
                                                     params.at(Names::Gain_Out),
                                                     NormalisableRange<float>(-24.f, 24.f, 0.5f, 1.f),
                                                     0));
    layout.add(std::make_unique<AudioParameterFloat>(params.at(Names::Threashold_Low_Band),
                                                     params.at(Names::Threashold_Low_Band),
                                                     NormalisableRange<float>(MIN_THREASHOLD,MAX_DECIBELS,1,1),
                                                     0));
    layout.add(std::make_unique<AudioParameterFloat>(params.at(Names::Threashold_Mid_Band),
                                                     params.at(Names::Threashold_Mid_Band),
                                                     NormalisableRange<float>(MIN_THREASHOLD, MAX_DECIBELS, 1, 1),
                                                     0));
    layout.add(std::make_unique<AudioParameterFloat>(params.at(Names::Threashold_High_Band),
                                                     params.at(Names::Threashold_High_Band),
                                                     NormalisableRange<float>(MIN_THREASHOLD, MAX_DECIBELS, 1, 1),
                                                     0));
...
{
    return new MultibandAudioCompressorAudioProcessor();
}
